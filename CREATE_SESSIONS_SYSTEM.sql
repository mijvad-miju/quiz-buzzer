-- Complete session-based quiz system setup
-- This creates sessions, links questions to sessions, and adds leaderboard functionality
...
-- Step 14: Create sample session and questions for testing
INSERT INTO public.sessions (session_name, session_number, is_active) 
VALUES ('Sample Session 1', 1, true);

-- Get the session ID for sample questions
DO $$
DECLARE
  sample_session_id UUID;
BEGIN
  SELECT id INTO sample_session_id FROM public.sessions WHERE session_name = 'Sample Session 1';
  
  -- Insert sample questions
  INSERT INTO public.questions (session_id, question_text, order_index) VALUES
  (sample_session_id, 'What is the capital of France?', 1),
  (sample_session_id, 'Which planet is known as the Red Planet?', 2),
  (sample_session_id, 'What is 2 + 2?', 3);
END $$;

-- Step 15: Verify the setup
SELECT 'Sessions created:' as status, COUNT(*) as count FROM public.sessions;
SELECT 'Questions created:' as status, COUNT(*) as count FROM public.questions;
